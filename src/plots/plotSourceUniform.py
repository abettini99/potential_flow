...
def plotSourceUniform(Lx, Ly, sourceStrength, Vinf):
    ...
    # Get velocities
    Vr       =  Vinf*np.cos(theta) + sourceStrength/(2*np.pi*r)
    Vtheta   = -Vinf*np.sin(theta)
    ...
    phi      = Vinf*r*np.cos(theta) + sourceStrength/(2*np.pi) * np.log(r)
    psi      = Vinf*r*np.sin(theta) + sourceStrength/(2*np.pi) * theta

    # Get body surface (determined by streamline in which stagnation point occurs)
    # We only do it for points not at stagnation since stagnation is of form 0/0
    # We also inject the stagnation point into array we are calculating (rS will go to 1, but evaluate as 0/0)
    n        = 100
    thetaS   = np.linspace(0+1e-6, 2*np.pi-1e-6, n)
    rS       = (sourceStrength/2 - sourceStrength/(2*np.pi)*thetaS)/(Vinf*np.sin(thetaS))
    # Inject stagnation points
    thetaS   = np.append(thetaS, np.pi)
    rS       = np.append(rS, sourceStrength/(2*np.pi*Vinf))
    ...
    VrS      =  Vinf*np.cos(thetaS) + sourceStrength/(2*np.pi*rS)
    VthetaS  = -Vinf*np.sin(thetaS)
    ...
    if sourceStrength != 0:
        fig1.add_trace(go.Scatter(name=f"Source",
                                  ...
                                  hovertemplate=f'<b>Source</b>'+
                                                 '<br>x = %{x:.4f}'+
                                                 '<br>y = %{y:.4f}'+
                                                f'<br>Strength = {sourceStrength:.4e}'+
                                                 '<extra></extra>',
                                  showlegend=False,
                                 ),
                       row=[1,1,1], col=[1,2,3]
    ...
    if sourceStrength != 0:
        fig1.add_trace(go.Scatter(name=f"Stagnation Point",
                                  x=[-sourceStrength/(2*np.pi*Vinf)], y=[0],
                                  mode='markers',
        ...
        fig4.add_trace(go.Scatter(name=f"Stagnation Point",
                                  x=[-sourceStrength/(2*np.pi*Vinf)], y=[1],
                                  mode='markers',
                                  marker=dict(color="blue", size=8),
                                  hoverinfo='skip',
                                  showlegend=False,
